...
**Теги, инвалидация и точечные правки кэша.** Создание и удаление инвалидируют тег списка -
данные должны прийти с сервера. А вот обновление товара список не инвалидирует: раньше оно было
завязано на теги по каждому товару, и правка одной карточки заставляла `infiniteQuery`
перезапрашивать **все** загруженные страницы - при трёх подгруженных это три запроса подряд.
Теперь [`updateProduct`](src/shared/api/products/api.ts) правит кэш вручную через
`updateQueryData` по всем активным аргументам списка, а при ошибке откатывает патч. В сеть уходит
один `PATCH`, список не моргает.
...
- Оптимистичные правки кэша для создания и удаления: пока они инвалидируют список целиком.
